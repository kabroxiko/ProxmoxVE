...
DB_PATH="/usr/local/kabroxiko/filebrowser.db"
DEFAULT_PORT=8080
...
ExecStartPre=/bin/touch /usr/local/kabroxiko/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/kabroxiko/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/kabroxiko/filebrowser.db -p ${PORT}
Restart=always
